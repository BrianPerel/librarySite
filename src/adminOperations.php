...
<?php 
	...
	if(isset($_POST['deleteItem']) || isset($_GET['delMessage'])) { ?>
		...
		<form action='deleteItem.php' action='POST' id="myForm" autocomplete="off" style='background-color: #DCDCDC'>
			<label>Name of item to be deleted: </label><br>
			<input autofocus name="name" type="text" size="40" placeholder="Item Name" id="name" onkeypress='memSort(event)' autocomplete="off" required></input>
	...
	if(isset($_POST['addItem'])) {
		...
		echo "<form action='addItem.php' action='POST' style='border: solid 0.1px; margin: 1% 35% 1% 35%; padding: 2% 0% 2% 0%; background-color: #DCDCDC' autocomplete='off'>";
			...
			echo '<input name="item_name" type="text" placeholder="Item Name" autofocus autocomplete="off" required></input><br><br>';
			
			echo '<label>Author: </label><br>';
			echo '<input name="author" type="text" placeholder="Author" autocomplete="off" required></input><br><br>'; 
			
			echo '<label>ISBN: </label><br>';
			echo '<input name="ISBN" type="number" placeholder="ISBN" autocomplete="off" required></input><br><br>'; 
			
			echo '<label>Publication Information: </label><br>';
			echo '<input name="pub_info" type="text" placeholder="Pub. Info" autocomplete="off" required></input><br><br>'; 
			
			echo '<label>Year Released: </label><br>';
			echo '<input name="year_released" type="number" placeholder="Year Released" autocomplete="off" required></input><br><br>'; 
			
			echo '<label>General Audience: </label><br>';
			echo '<input name="gen_aud" type="text" placeholder="Gen. Audience" autocomplete="off" required></input><br><br>'; 
						
			echo '<label>Summary: </label><br>';
			echo '<textarea name="summary" type="text" placeholder="Summary" rows="3" cols="30" autocomplete="off" required></textarea><br><br>'; 
			
			echo '<label>Item Type: </label><br>';
			echo '<input name="item_type" type="text" placeholder="Item Type" autocomplete="off" required></input><br><br>'; 
			
			echo '<label>Col. Number: </label><br>';
			echo '<input name="col_no" type="text" placeholder="Col. Number" autocomplete="off" required></input><br><br>'; 
			
			echo '<label>Price: </label><br>';
			echo '<input name="price" type="text" placeholder="Price ($)" autocomplete="off" required></input><br><br>'; 
	...
	# if add message received from addItem.php go here 
